...
function findWinner(drawings, boards) {
  ...
  while(drawingCount < drawings.length && !winningBoard) {
    numberDrawn = drawings[drawingCount];
    for(let i=0;i<boards.length;i++) {
      const hasMatch = markMatch(boards[i], numberDrawn);
      if (hasMatch && checkWinner(boards[i])) {
        winningBoard = boards[i];
        console.log({numberDrawn, drawingCount})
        break;
      }
    }
    drawingCount++;
  ...
  return {winningBoard, winningDraw: numberDrawn};
}

const {winningBoard, winningDraw} = findWinner(numbersToDraw, gameBoards);

console.table(winningBoard)
...
const unmarkedCellsByRow = winningBoard.map(r => sum(numericValueOfCells(findUnmarkedCells(r))));
